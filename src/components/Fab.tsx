// This file is copied from @react-native-material/core 1.3.7, fixing the bug reported
// at https://github.com/yamankatby/react-native-material/issues/53
...
export interface FABProps
  ...
  icon?:
    | React.ReactElement
    | ((props: {color: string; size: number}) => React.ReactElement | null)
    | null;

  label?:
    | string
    | React.ReactElement
    | ((props: {color: string}) => React.ReactElement | null)
    | null;
...
const FAB: React.FC<FABProps> = ({
  ...
  useEffect(() => {
    Animated.timing(animated, {
      toValue: visible ? 1 : 0,
      duration: 200,
      // todo fix https://github.com/yamankatby/react-native-material/issues/53
      // https://github.com/yamankatby/react-native-material/pull/55/files
  ...
  const getLoadingIndicator = () => {
    if (!loadingIndicator) {
      return <ActivityIndicator color={palette.on} />;
    }
  ...
  const getIcon = () => {
    if (loading && loadingIndicatorPosition === 'icon') {
      return getLoadingIndicator();
    }
    return typeof icon === 'function'
      ? icon({color: palette.on, size: 24})
      : icon;
  };
      ...
      case 'string':
        return (
          <Text
            variant="button"
            selectable={false}
            style={[{color: palette.on}, labelStyle]}>
            {label}
          </Text>
        );
      case 'function':
